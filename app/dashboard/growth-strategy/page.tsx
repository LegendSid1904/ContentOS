...
function ScoreGauge({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="font-mono text-[12px] text-tx-3 uppercase tracking-wider w-28 flex-shrink-0">{label}</span>
...
export default function GrowthStrategyPage({ appId: _appId }: any = {}) {
  return (
    <Suspense fallback={<GrowthFallback />}>
      <GrowthStrategyContent appId={_appId} />
    </Suspense>
  );
}
...
function GrowthStrategyContent({ appId: propAppId }: { appId?: string | null }) {
  ...
  function exportPDF() {
    if (!data) return;
    const html = `<!DOCTYPE html><html><head><title>Growth Strategy - ${niche}</title><style>
      @page { margin: 0.75in; }
    ...
    <div className="max-w-3xl space-y-6 relative z-10">
      ...
      <div className="crt-monitor relative crt-brackets">
        ...
        <div className="crt-monitor-content p-6 space-y-6">
          ...
          {error && (
            <div className="font-mono text-[12px] text-err bg-err/10 border border-err/20 rounded-r3 p-3">
          ...
          {isResultsStep && data && (
            <div ref={outputRef} className="space-y-6">
              <div className="flex items-center justify-between reveal d1">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={exportPDF} className="btn-terminal text-[11px]">
                    {"[PDF]"}
                  </button>
                  <button onClick={handlePersona} disabled={personaLoading} className="btn-terminal text-[11px]">
                    {personaLoading ? "..." : "[PERSONA]"}
                  </button>
                  <button onClick={handleEngagement} disabled={engagementLoading} className="btn-terminal text-[11px]">
                    {engagementLoading ? "..." : "[ENGAGE]"}
                  </button>
                  <button onClick={handleSave} className="btn-terminal text-[11px]">
                    {"[SAVE]"}
                  </button>
                  <button onClick={handleGenerate} className="btn-terminal text-[11px]">
                    {"[REGEN]"}
                  </button>
                </div>
              </div>
              ...
              <div className="crt-monitor relative crt-brackets reveal d2" style={{ background: "rgba(0,0,0,0.25)" }}>
                <div className="crt-micro-tl">
                  ...
                  <span className="text-tx-4">|</span>
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase">scorecard</span>
                </div>
                <div className="crt-monitor-header">
                  <span className="w-2 h-2 rounded-full bg-vi-400/60" />
                  <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">Growth Audit</span>
                </div>
                <div className="crt-monitor-content p-4 space-y-3">
                  ...
                  <div className="pt-2 border-t border-white/[0.04]">
                    <span className="font-mono text-[11px] text-ok uppercase tracking-wider">Strengths</span>
                    {data.audit.strengths.map((s, i) => (
                      <div key={i} className="flex items-start gap-2 mt-1">
                        <span className="font-mono text-[11px] text-ok mt-0.5">+</span>
                        <span className="font-mono text-[12px] text-tx-1">{s}</span>
                      </div>
                    ))}
                  </div>
                  <div className="pt-2 border-t border-white/[0.04]">
                    <span className="font-mono text-[11px] text-err uppercase tracking-wider">Weaknesses</span>
                    {data.audit.weaknesses.map((w, i) => (
                      <div key={i} className="flex items-start gap-2 mt-1">
                        <span className="font-mono text-[11px] text-err mt-0.5">&mdash;</span>
                        <span className="font-mono text-[12px] text-tx-1">{w}</span>
                      </div>
                    ))}
                  </div>
                  <div className="pt-2 border-t border-white/[0.04]">
                    <span className="font-mono text-[11px] text-te-400 uppercase tracking-wider">Growth Levers</span>
                    {data.audit.growth_levers.map((gl, i) => (
                      <div key={i} className="flex items-start gap-2 mt-1">
                        <span className="font-mono text-[11px] text-te-400 mt-0.5">{">>"}</span>
                        <span className="font-mono text-[12px] text-tx-1">{gl}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="crt-monitor relative crt-brackets reveal d3" style={{ background: "rgba(0,0,0,0.25)" }}>
                <div className="crt-micro-tl">
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-te-400/60">plan</span>
                  <span className="text-tx-4">|</span>
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase">90_day</span>
                </div>
                <div className="crt-monitor-header">
                  <span className="w-2 h-2 rounded-full bg-te-400/60" />
                  <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">90-Day Plan ({data.plan.length} weeks)</span>
                </div>
                  ...
                  {data.plan.map((week) => (
                    <div key={week.week_number} className="pb-3 border-b border-white/[0.04] last:border-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-[11px] text-fu-400 tracking-wider">W{String(week.week_number).padStart(2, "0")}</span>
                        <span className="font-mono text-[11px] text-tx-1 font-semibold">{week.theme}</span>
                      </div>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {week.content_focus.map((cf, j) => (
                          <span key={j} className="font-mono text-[11px] text-tx-2 bg-white/[0.03] px-1.5 py-0.5 rounded-r2">{cf}</span>
                        ))}
                      </div>
                      <p className="font-mono text-[12px] text-te-400 mt-1">&gt; {week.growth_tactic}</p>
                      <p className="font-mono text-[11px] text-tx-3 italic mt-0.5">Milestone: {week.milestone}</p>
                    </div>
                  ))}
                </div>
                <div className="crt-micro-bl">
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-ok">PLAN READY</span>
                </div>
              </div>

              <div className="crt-monitor relative crt-brackets reveal d4" style={{ background: "rgba(0,0,0,0.25)" }}>
                <div className="crt-micro-tl">
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-te-400/60">monetize</span>
                  <span className="text-tx-4">|</span>
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase">roadmap</span>
                </div>
                <div className="crt-monitor-header">
                  <span className="w-2 h-2 rounded-full bg-fu-400/60" />
                  <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">Monetization Roadmap</span>
                </div>
                <div className="crt-monitor-content p-4 space-y-4">
                  {data.monetization.map((phase, i) => (
                    <div key={i} className="pb-3 border-b border-white/[0.04] last:border-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-[11px] text-vi-400 tracking-wider uppercase">{phase.phase}</span>
                        <span className="font-mono text-[11px] text-tx-3">({phase.timeframe})</span>
                      </div>
                      <div className="space-y-0.5 mt-1">
                        {phase.tactics.map((t, j) => (
                          <div key={j} className="flex items-start gap-2">
                            <span className="font-mono text-[11px] text-te-400 mt-0.5">&bull;</span>
                            <span className="font-mono text-[12px] text-tx-1">{t}</span>
                          </div>
                        ))}
                      </div>
                      <p className="font-mono text-[11px] text-ok mt-1">Target: {phase.revenue_target}</p>
                    </div>
                  ))}
                </div>
                <div className="crt-micro-bl">
                  <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-ok">ROADMAP READY</span>
                </div>
              </div>
              ...
              {data.algorithm_tips.length > 0 && (
                <div className="crt-monitor relative crt-brackets reveal d5" style={{ background: "rgba(0,0,0,0.25)" }}>
                  <div className="crt-micro-tl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-te-400/60">tips</span>
                    <span className="text-tx-4">|</span>
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase">algorithm</span>
                  </div>
                  <div className="crt-monitor-header">
                    <span className="w-2 h-2 rounded-full bg-ok/60" />
                    <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">Algorithm Tips</span>
                  </div>
                  <div className="crt-monitor-content p-4 space-y-2">
                    {data.algorithm_tips.map((tip, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className="font-mono text-[11px] text-te-400 mt-0.5">{">>"}</span>
                        <span className="font-mono text-[12px] text-tx-1">{tip}</span>
                      </div>
                    ))}
                  </div>
                  <div className="crt-micro-bl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-ok">TIPS READY</span>
                  </div>
                </div>
              )}
              ...
              {persona && (
                <div className="crt-monitor relative crt-brackets reveal" style={{ background: "rgba(0,0,0,0.25)" }}>
                  <div className="crt-micro-tl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-te-400/60">audience</span>
                    <span className="text-tx-4">|</span>
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase">persona</span>
                  </div>
                  <div className="crt-monitor-header">
                    <span className="w-2 h-2 rounded-full bg-fu-400/60" />
                    <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">{persona.name}</span>
                  </div>
                  <div className="crt-monitor-content p-4 space-y-3">
                    <div>
                      <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Demographics</span>
                      <p className="font-mono text-[12px] text-tx-1">{persona.demographics}</p>
                    </div>
                    <div>
                      <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Psychographics</span>
                      {persona.psychographics.map((p, i) => (
                        <div key={i} className="flex items-start gap-2 mt-0.5">
                          <span className="font-mono text-[11px] text-te-400 mt-0.5">&bull;</span>
                          <span className="font-mono text-[12px] text-tx-1">{p}</span>
                        </div>
                      ))}
                    </div>
                    <div>
                      <span className="font-mono text-[11px] text-err uppercase tracking-wider">Pain Points</span>
                    ...
                    <div>
                      <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Content Preferences</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {persona.content_preferences.map((p, i) => (
                          <span key={i} className="font-mono text-[11px] text-tx-2 bg-white/[0.03] px-1.5 py-0.5 rounded-r2">{p}</span>
                        ))}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-white/[0.04]">
                      <div>
                        <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Best Time</span>
                        <p className="font-mono text-[12px] text-te-400">{persona.best_time_to_post}</p>
                      </div>
                      <div>
                        <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Language/Tone</span>
                        <p className="font-mono text-[12px] text-tx-1">{persona.language_tone}</p>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-white/[0.04]">
                      <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Platforms</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {persona.platforms_frequent.map((p, i) => (
                          <span key={i} className="font-mono text-[11px] text-vi-400 bg-vi-400/10 px-1.5 py-0.5 rounded-r2">{p}</span>
                        ))}
                      </div>
                    </div>
                    <div className="pt-2 border-t border-white/[0.04]">
                      <span className="font-mono text-[11px] text-tx-3 uppercase tracking-wider">Influencers They Follow</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {persona.influencers_they_follow.map((inf, i) => (
                          <span key={i} className="font-mono text-[11px] text-fu-400/80 bg-fu-400/10 px-1.5 py-0.5 rounded-r2">{inf}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="crt-micro-bl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-ok">PERSONA READY</span>
                  </div>
                </div>
              )}
              ...
              {engagement && (
                <div className="crt-monitor relative crt-brackets reveal" style={{ background: "rgba(0,0,0,0.25)" }}>
                  <div className="crt-micro-tl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-te-400/60">engage</span>
                    <span className="text-tx-4">|</span>
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase">prompts</span>
                  </div>
                  <div className="crt-monitor-header">
                    <span className="w-2 h-2 rounded-full bg-te-400/60" />
                    <span className="font-mono text-[11px] font-semibold text-tx-1 tracking-tight ml-2">Engagement Prompts</span>
                  </div>
                  <div className="crt-monitor-content p-4 space-y-4">
                    <div>
                      <span className="font-mono text-[11px] text-te-400 uppercase tracking-wider">DM Scripts</span>
                      {engagement.dm_scripts.map((dm, i) => (
                        <div key={i} className="mt-2 pb-2 border-b border-white/[0.04] last:border-0">
                          <span className="font-mono text-[11px] text-tx-3">{dm.scenario}</span>
                          <p className="font-mono text-[12px] text-tx-1 mt-0.5">{dm.script}</p>
                        </div>
                      ))}
                    </div>
                    <div className="pt-2 border-t border-white/[0.04]">
                      <span className="font-mono text-[11px] text-vi-400 uppercase tracking-wider">Comment Templates</span>
                      {engagement.comment_templates.map((ct, i) => (
                        <div key={i} className="mt-2 pb-2 border-b border-white/[0.04] last:border-0">
                          <span className="font-mono text-[11px] text-tx-3">[{ct.type}]</span>
                          <p className="font-mono text-[12px] text-tx-1 mt-0.5">{ct.template}</p>
                        </div>
                      ))}
                    </div>
                    <div className="pt-2 border-t border-white/[0.04]">
                      <span className="font-mono text-[11px] text-fu-400 uppercase tracking-wider">CTA Frameworks</span>
                      {engagement.cta_frameworks.map((cta, i) => (
                        <div key={i} className="mt-2 pb-2 border-b border-white/[0.04] last:border-0">
                          <span className="font-mono text-[11px] text-tx-3 font-semibold">{cta.name}</span>
                          <p className="font-mono text-[12px] text-tx-1 mt-0.5">{cta.framework}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="crt-micro-bl">
                    <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-ok">PROMPTS READY</span>
                  </div>
                </div>
              )}
